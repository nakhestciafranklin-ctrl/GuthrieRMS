...
function locations(div){let all=db.settings?.inventoryLocations||[]; if(!all.length)return div==='bistro'?['Bistro Dry Storage','Bistro Beverage Station','Bistro Reach-In Cooler','Bistro Freezer','FOH Storage']:['Kitchen 1','Kitchen 2','Kitchen 3','Bakeshop','Catering Kitchen','Dry Storage','Fine Dining Storage','Walk-In Fridge','Walk-In Freezer']; return div==='bistro'?all.filter(x=>/bistro|foh/i.test(x)):all.filter(x=>!/bistro|foh/i.test(x))}
...
function catering(){
  ...
  <div class="catering-guest-block"><label>Guest Count / Quantity</label><input id="catGuests" type="number" class="input" value="25" min="1" oninput="previewCatering()"><span class="small">Enter the number of guests or package units for this event.</span></div>
