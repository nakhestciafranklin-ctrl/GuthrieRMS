...
let state={user:null,view:'dashboard',pin:'',activeOrder:null,seat:1,checkoutType:'table',selectedOrder:null,invDivision:'bistro',kmsStation:'all'};
...
function normalizeAccessForUser(u){
  const roleDefaults={
    manager:['clock','dining','quick','checkout','kms','inventory','catering','recipes','reports','invoices','development','closeout','setup'],
    teacher:['clock','inventory','catering','recipes','reports','invoices','development'],
...
function topbar(){let labels={clock:'Clock In/Out',dining:'Dining Room',quick:'Counter + To-Go',checkout:'Checkout',kms:'KMS',inventory:'Inventory',catering:'Catering',recipes:'Recipes/Labs',recipes:'Recipes/Labs',reports:'Reports',invoices:'Invoices',development:'Student Development',setup:'Setup',closeout:'End of Day'}; let nav=allowedViews().filter(v=>labels[v]).map(v=>v+':'+labels[v]); return `<header class="topbar"><img src="assets/guthrie-logo.png"><b>Guthrie RMS</b><span>${state.user.name} • ${state.user.pos}</span><span class="spacer"></span><nav class="nav"><button data-view="dashboard" class="${state.view==='dashboard'?'active':''}">🏠 Dashboard</button>${nav.map(x=>{let [v,l]=x.split(':');return `<button data-view="${v}" class="${state.view===v?'active':''}">${l}</button>`}).join('')}<button data-logout>Log Out</button></nav></header>`}
function bindCommon(){
...
function roleTiles(){let tiles=allowedViews(); return tiles.map(v=>`<button class="tile" data-view="${v}">${({clock:'Clock In / Out',dining:'Dining Room',quick:'Counter + To-Go',checkout:'Checkout',kms:'KMS',inventory:'Inventory',catering:'Catering',recipes:'Recipes/Labs',recipes:'Recipes/Labs',reports:'Reports',invoices:'Invoices',development:'Student Development',setup:'Setup Center',closeout:'End of Day Closeout'})[v]}</button>`).join('')}
const views={dashboard:()=>`<section class="card"><h1>Welcome, ${state.user.name}</h1><p>${activeShift()?'Clocked in: '+new Date(activeShift().in).toLocaleTimeString():'Not clocked in'}</p><div class="grid">${roleTiles()}</div></section>`,
...
dining:()=>dining(), quick:()=>quick(), order:()=>orderScreen(), checkout:()=>checkout(), kms:()=>kms(), inventory:()=>inventory(), catering:()=>catering(), recipes:()=>recipesLab(), reports:()=>reports(), invoices:()=>invoiceCenter(), development:()=>development(), closeout:()=>closeout(), setup:()=>setup()};
...
function setup(){if(state.user.role!=='manager')return `<section class="card"><h1>Setup Locked</h1><p>Only managers can change system setup or grant additional access.</p></section>`; return `<section class="card"><h1>Setup Center</h1><p class="small">Manager only. Teachers default to Culinary Department information. Managers may grant additional access as needed.</p>
<h2>System Mode</h2><div class="row"><button class="primary" onclick="toggleDemoMode()">${db.settings?.demoMode?'Turn Demo Mode OFF':'Turn Demo Mode ON'}</button><span class="pill">Current: ${db.settings?.demoMode?'Demo Mode':'Production Mode'}</span></div><h2>Dining Room Setup</h2><div class="row"><label>Number of Tables <input id="tableCount" class="input" type="number" min="1" value="${db.tables.length}"></label><label>Default Seats Per Table <input id="seatCount" class="input" type="number" min="1" value="4"></label><button class="primary" onclick="resetTables()">Apply Dining Room Setup</button></div>
<h2>Editable Positions</h2><div class="row"><input id="newPos" class="input" placeholder="New position name"><button class="primary" onclick="addPosition()">Add Position</button></div><div class="modifier-list section">${db.positions.map((p,i)=>`<button title="Click to rename" onclick="renamePosition(${i})">${p}</button>`).join('')}</div>
${bistroMenuSetupPanel()}<h2>Student Access / Roster</h2><p class="small">Add each student with name, assigned teacher, position, and student ID. The student ID automatically becomes the login PIN.</p><button class="primary" onclick="addStudentUser()">Add Student</button>${userTable('student')}
<h2>Teacher Setup</h2><p class="small">Add teacher name and unique PIN. Teachers default to Culinary Department access only. Managers can add extra access below.</p><div class="row"><input id="teacherName" class="input" placeholder="Teacher name"><input id="teacherPin" class="input" placeholder="Teacher PIN"><button class="primary" onclick="addTeacherFromForm()">Add Teacher</button></div>${userTable('teacher')}
<h2>Manager PINs</h2>${userTable('manager')}
<div class="section panel"><h2>Recipe & Culinary Lab Setup Restored</h2><p class="small">Recipe entry is now available again through the Recipes/Labs page. Teachers can build culinary lab recipes with ingredient lines connected to Culinary Inventory, then deduct inventory when labs are completed.</p><button class="primary" onclick="state.view='recipes';render()">Open Recipes/Labs</button></div></section>`}
...
window.addTeacherFromForm=()=>{let name=($('#teacherName')?.value||'').trim(); let pin=($('#teacherPin')?.value||'').trim(); if(!name){alert('Teacher name is required.');return;} if(!pin){alert('Teacher PIN is required.');return;} if(db.users.some(u=>u.pin===pin)){alert('That PIN is already assigned. Please choose a unique PIN.');return;} db.users.push({id:Date.now(),name,pin,role:'teacher',pos:'Instructor',active:true,access:['clock','inventory','catering','recipes','reports','invoices','development'],inventoryScope:'culinary'}); save(); toast('Teacher added with Culinary Department access.'); render();};

window.addUser=role=>{let nextPin=prompt(`Enter unique ${role} PIN:`); if(!nextPin)return; if(db.users.some(u=>u.pin===nextPin)){alert('That PIN is already assigned. Please choose a unique PIN.');return;} let name=prompt(`Enter ${role} name:`)||`New ${role}`; let pos=role==='teacher'?'Instructor':role==='manager'?'Manager':'Server'; let access=role==='teacher'?['clock','inventory','catering','recipes','reports','invoices','development']:(role==='manager'?['clock','dining','quick','checkout','kms','inventory','catering','reports','closeout','setup']:['clock','dining','quick','checkout','kms','inventory','development']); let inventoryScope=role==='teacher'?'culinary':(role==='manager'?'all':'assigned'); db.users.push({id:Date.now(),name,pin:nextPin,role,pos,active:true,access,inventoryScope});save();render()};
